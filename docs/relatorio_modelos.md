...
|-----------|-------|---------------|
| `temperature` | 0.2 | respostas factuais sobre produtos exigem baixa criatividade; reduz alucinação de specs |
| `top_p` | 0.9 | corte de nucleus sampling padrão; com temperatura baixa, mantém fluidez sem divagar |
| `max_tokens` (`num_predict`) | 512 | respostas curtas (máx. 3 parágrafos no prompt v2); limita custo e latência |
| memória (`max_token_limit`) | 1200 | ~6–8 turnos de conversa; acima disso a poda remove os turnos mais antigos |

Valores lidos do `.env` — ver `.env.example`.

## Resultados

### Latência e tokens (multi-provider: 2 modelos × 2 prompts × 3 perguntas)
...
_(preencher após a rodada real: qual modelo ficou no projeto e por quê — qualidade vs. latência vs. custo)_
