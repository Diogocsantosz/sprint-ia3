...
def rodar_lcel(
    casos: list[dict], cfg, mock: bool, versao_prompt: str, provider: str
) -> list[dict]:
    bot = AssistenteEV(
        cfg,
        backend="mock" if mock else provider,
...
def main() -> None:
    ...
    cfg = carregar_config()
    if args.modelo:
        if args.versao == "lcel" and args.provider == "groq":
            cfg.modelo_groq = args.modelo
        else:
            cfg.modelo_principal = args.modelo
    if args.versao == "lcel" and args.provider == "groq" and not os.getenv("GROQ_API_KEY"):
        ap.error("GROQ_API_KEY não configurada. Adicione a chave ao arquivo .env.")
    ...
    if args.versao == "lcel":
        resultados = rodar_lcel(casos, cfg, args.mock, args.prompt, args.provider)
        resumo = resumir(resultados, versao_prompt=args.prompt)
        modelo_usado = (
            "mock-ev" if args.mock
            else args.modelo or (cfg.modelo_groq if args.provider == "groq" else cfg.modelo_principal)
        )
        if not args.mock and args.provider != "ollama":
            modelo_chave = modelo_usado.replace(":", "_").replace("/", "_")
            chave = f"lcel_{args.provider}_{modelo_chave}_{args.prompt}"
        elif args.modelo:
    ...
    provider_usado = "mock" if args.mock else (
        "ollama" if args.versao == "legado" else args.provider
    )
    modelo_usado = "mock-ev" if args.mock else (
        cfg.modelo_principal if args.versao == "legado"
        else args.modelo or (cfg.modelo_groq if args.provider == "groq" else cfg.modelo_principal)
    )
